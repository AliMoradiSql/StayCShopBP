(function ($) {
...

            {
                ...
                render: (data, type, row, meta) => {
                    return [
                        ...
                        '   </button>',
                        `   <button type="button" class="btn btn-sm bg-danger delete-info" data-info-id="${row.id}" data-info-name="${row.discountName}">`,
    ...
    $(document).on('click', '.edit-info', function (e) {
        ...
        e.preventDefault();
        abp.ajax({
            url: abp.appPath + 'Discounts/CreateOrEditDiscountModal?Id=' + objId,
            type: 'Get',
            dataType: 'html',
    ...
    abp.event.on('Discount.edited', (data) => {
        getData();
    });
